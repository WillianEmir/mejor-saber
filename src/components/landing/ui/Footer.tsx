    ...
    <footer className="bg-gray-100" aria-labelledby="footer-heading">
      <div className="container mx-auto py-12 px-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 items-start">
          ...
          <div className="space-y-4 text-center md:text-left">
            ...
            <ul className="space-y-2">
              {navigation.map((item) => (
                <li key={item.name}>
                  <Link href={item.href} className="text-sm text-gray-700 hover:text-indigo-600">
                    {item.name}
                  </Link>
                </li>
              ))}
        ...
        <div className="mt-12 border-t border-gray-300 pt-6 text-center">
          <p className="text-xs text-gray-500">&copy; {new Date().getFullYear()} Saber Ya, Inc. Todos los derechos reservados.</p>
        </div>
      </div>
    </footer>
  )
