...
function SubmitButton({ editMode }: { editMode: boolean }) {
  ...
  return (
    ...
    >
      {pending ? (editMode ? 'Guardando...' : 'Creando...') : (editMode ? 'Guardar Cambios' : 'Crear Evidencia')}
    </button>
  )
}

export default function EvidenciaModal({ isOpen, onClose, afirmacionId, evidencia }: Props) {
            ...
            >
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all dark:bg-gray-800">
                ...
                <form ref={formRef} action={dispatch} className="mt-4 space-y-4">
                  {editMode && <input type="hidden" name="id" value={evidencia.id} />}
