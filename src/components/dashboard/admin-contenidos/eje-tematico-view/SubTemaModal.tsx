...
import { createOrUpdateSubTema } from '@/src/lib/actions/subTema.action' 
...
function SubmitButton({ editMode }: { editMode: boolean }) {
  ...
  return (
    ...
    >
      {pending ? (editMode ? 'Guardando...' : 'Creando...') : (editMode ? 'Guardar Cambios' : 'Crear Subtema')}
    </button>
  )
}
...
export default function SubTemaModal({ isOpen, onClose, seccionId, ejeTematicoId, subTema }: Props) {
  ...
  const [imageUrl, setImageUrl] = useState(subTema?.imagen || '')

  const initialState = { message: null, errors: {} }
  const [state, dispatch] = useActionState(createOrUpdateSubTema, initialState)
          ...
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            >
              ...
              <form ref={formRef} action={dispatch} className="mt-4 space-y-4">
                {editMode && <input type="hidden" name="id" value={subTema.id} />}
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <input id="nombre" name="nombre" type="text" defaultValue={subTema?.nombre || ''} required  className='border border-gray-300 rounded-md p-2 w-full'/>
                    {state?.errors?.nombre && <p>{state.errors.nombre[0]}</p>}
                  </div>
                  <div>
                    <label htmlFor="video">URL del Video</label>
                    <input id="video" name="video" type="text" defaultValue={subTema?.video || ''} className='border border-gray-300 rounded-md p-2 w-full'/>
                    {state?.errors?.video && <p>{state.errors.video[0]}</p>}
                  </div>
                </div>

                <div>
                  <label htmlFor="descripcion">Descripción</label>
                  <textarea id="descripcion" name="descripcion" rows={3} defaultValue={subTema?.descripcion || ''} className='border border-gray-300 rounded-md p-2 w-full'/>
                  {state?.errors?.descripcion && <p>{state.errors.descripcion[0]}</p>}
                </div>

                <div>
                  <label htmlFor="ejemplo">Ejemplo</label>
                  <textarea id="ejemplo" name="ejemplo" rows={3} defaultValue={subTema?.ejemplo || ''} className='border border-gray-300 rounded-md p-2 w-full'/>
                  {state?.errors?.ejemplo && <p>{state.errors.ejemplo[0]}</p>}
                </div>
