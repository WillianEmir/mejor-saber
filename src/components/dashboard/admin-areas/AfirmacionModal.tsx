...
// --- Tipos de Datos ---
type Competencia = { id: string; nombre: string };
type Afirmacion = { id: string; nombre: string; competenciaId: string };

interface AfirmacionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: { nombre: string; competenciaId: string; id?: string }) => void;
  initialData?: Afirmacion | null;
  // Lista de Competencias para el <select>
  competencias: Competencia[];
}

export const AfirmacionModal: React.FC<AfirmacionModalProps> = ({ isOpen, onClose, onSubmit, initialData, competencias }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (isEditMode && initialData) {
        ...
        setCompetenciaId(initialData.competenciaId);
      } else {
        setNombre('');
        setCompetenciaId(competencias[0]?.id || '');
      }
    }
  }, [initialData, isEditMode, isOpen, competencias]);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!nombre.trim() || !competenciaId) {
      alert('Todos los campos son obligatorios.');
      return;
    }
    onSubmit({ nombre, competenciaId, id: initialData?.id });
    onClose();
  };
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            <Dialog.Panel className="w-full max-w-md transform rounded-2xl bg-white dark:bg-gray-800 p-6 text-left align-middle shadow-xl transition-all">
              <Dialog.Title as="h3" className="text-lg font-medium text-gray-900 dark:text-gray-100">
                {isEditMode ? 'Editar Afirmación' : 'Agregar Afirmación'}
              </Dialog.Title>
              ...
              <form onSubmit={handleSubmit} className="mt-4 space-y-4">
                <div>
                  <label htmlFor="competencia-select" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    Competencia
                  </label>
                  <select
                ...
                <div>
                  <label htmlFor="afirmacion-nombre" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    Texto de la Afirmación
                  </label>
                  <textarea
                    id="afirmacion-nombre"
                    rows={3}
                    value={nombre}
