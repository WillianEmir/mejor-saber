...
interface Props {
  afirmacionId: AfirmacionType['id']
...
export default function AfirmacionModal({ afirmacionId, evidencia }: Props) {
  ...
  const addMode = searchParams.get('add-evidencia') === 'true'
  const editEvidenciaQuery = searchParams.get('edit-evidencia')
  const showModal = addMode || !!editEvidenciaQuery

  const onClose = useCallback(() => {
    const newSearchParams = new URLSearchParams(searchParams.toString())
    newSearchParams.delete('add-evidencia')
    newSearchParams.delete('edit-evidencia')
    const newPath = `${pathname}?${newSearchParams.toString()}`
    router.replace(newPath)
  }, [pathname, router, searchParams])

  const initialState = { message: null, errors: {} }
  const [state, dispatch] = useActionState(createOrUpdateEvidencia, initialState)
          ...
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all dark:bg-gray-800">
                ...
                >
                  {editEvidenciaQuery ? 'Editar Evidencia' : 'Agregar Nueva Evidencia'}
                </Dialog.Title>
                <button
                  type="button"
                  onClick={onClose}
                ...
                <form ref={formRef} action={dispatch} className="space-y-4">
                  ...
                  {!!editEvidenciaQuery && <input type="hidden" name="id" value={evidencia.id} />}

                  <input type="hidden" name="afirmacionId" value={afirmacionId} />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      id="nombre"
                      name="nombre"
                      defaultValue={!!editEvidenciaQuery ? evidencia.nombre : ''}
                  ...
                  <div className="flex justify-end gap-4 pt-4">
                    ...
                    <SubmitButton editMode={!!editEvidenciaQuery} />
                  </div>
                </form>
              </Dialog.Panel>
            </Transition.Child>
